 ...
 * Chapter 8:
 *    - Post processing
 *    - using effect composers and external shaders
 */
function init() {
  // scene
  ...
  // effectComposer
  /*
   * Using effect passes, there are two things that we need to remember to do.
   * The first thing is we need to set the renderToScreen attribute to true on
   *  the last effect that we have in our effects stack. Then I need to remember
   *  to add this effect inside the effect composer using the addPass method.
   *
   * Also, instead of passing the renderer inside the update function I will be
   * passing in the composer.
  */
  ...
  gui.add(vignetteEffect.uniforms['darkness'], 'value', 0, 10);
